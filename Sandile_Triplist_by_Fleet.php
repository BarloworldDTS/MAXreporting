...
/** Object::Sandile_Triplist_by_Fleet
 ...
 */
class Sandile_Triplist_by_Fleet {
	//: Constants
	...
	const DOC_FILE_NAME = "Sandile_Triplist_by_Fleet";
	const DOC_SUBJECT = "Exported MAX Report of Triplist by Fleet using API calls";
	...
	//: Public functions
	//: Accessors
	/** Sandile_Triplist_by_Fleet::getApiUrl()
	*   base url to call
	*   @return string
	*/
	...
	/** Sandile_Triplist_by_Fleet::getExcelFile()
	...
	/** Sandile_Triplist_by_Fleet::setExcelFile($_setFile)
	...
	/** Sandile_Triplist_by_Fleet::getFileName()
	 *  @return string: $this->_excelFileName
	 */
	...
	/** Sandile_Triplist_by_Fleet::setFileName($_setFile)
	 *  @param string: $_setFile
	 */
	...
	/** Sandile_Triplist_by_Fleet::writeExcelFile($excelFile, $excelData)
	...
	//: Magic
	/** Sandile_Triplist_by_Fleet::__construct()
	* Class constructor
	*/
	public function __construct() {
		try {
			...
			$reportDates = (array) array();
			$allDates = (array) $this->createDateRangeArray($startDate, $endDate);
			foreach ($allDates as $rDate) {
				$_dow = date("N", strtotime($rDate));
				switch ($_dow) {
					case "7";
					case "6";
						$reportDates[] = $rDate;
						$_date = strtotime("+1 year", strtotime($rDate));
						$reportDates[] = date("Y-m-d", $_date);
					break;
					default;
				}
			}
			...
			foreach ($reportDates as $value) {
				...
				$constructedDates[$aYear][$aMonth][date("d", strtotime($value))] = $value;
			}
			//: End
			...
			foreach ($constructedDates as $key1 => $value1) {
				foreach ($value1 as $key2 => $value2) {
					...
					$this->setExcelFile("/export/" . $this->getFileName() . "_" . $fleet);
	...
	/** Sandile_Triplist_by_Fleet::__destruct()
		* Class destructor
